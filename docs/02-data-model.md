...
- **Control-status decision engine (ADR-pending).** What deterministic rules read `evidence` rows and yield a `status` (`compliant` / `partial` / `non_compliant` / `not_applicable` / `unknown`). The Phase 2 check service ships with a stub `baseline_decider` that returns `'not_assessed'` for every control, so the SCD Type 2 transition mechanics are proven before the real engine exists. Distinct future ADR.
...
- **Questionnaire & evidence input model (ADR-pending).** The schema of the initial readiness questionnaire, how its answers materialise into `evidence` rows, and how uploaded documents feed the same pipeline. Drives the eventual decision engine above.
