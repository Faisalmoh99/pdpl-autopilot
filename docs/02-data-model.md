...
- **Readiness scoring algorithm — DECIDED, [ADR-0007](adr/0007-readiness-scoring-model.md) (2026-06-13).** Findings statuses + `controls.severity_weight` aggregate into a weighted **readiness score** (a maturity indicator, NOT a compliance %), reported **separately from COVERAGE** so a high score over thin coverage can never masquerade as compliance. `partial` = 0.5 credit; `unknown` = assessed/0.0; `not_assessed` excluded from the score (counts only against coverage); `not_applicable` excluded entirely; no assessed controls → `score = None`. Read as active controls LEFT JOIN current findings. A gap report lists every non-done control (non_compliant / partial / unknown / not-yet-assessed) ordered by severity. 100% deterministic; AI is never in the path. The score is computed on read and not persisted (no `scores` table yet), so it writes no `audit_log` row — the reserved `score.computed` event waits for persisted scoring.
