...
- **Prompt-version governance** — how `prompt_version` is bumped and reviewed when the prompt changes. For now it is a constant in `pdpl.ai`; a lightweight process can come when prompts change often. **RESOLVED in [ADR-0013](0013-prompt-version-governance.md): the trigger list, a single-counter scheme + changelog, the cache consequence, and a mechanical guard (`tests/test_prompt_version_governance.py`) that pins a hash of the real rendered prompt surface to `PROMPT_VERSION`.**
